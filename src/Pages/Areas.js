...
import CustomersTable from "../containers/AreaContainers/CustomersTable";
import { useDispatch, useSelector } from "react-redux";
import axios from "axios";
import { BiArrowBack } from "react-icons/bi";
import { BiDotsVerticalRounded } from "react-icons/bi";
import RegisterCustomers from "../containers/AreaContainers/RegisterCustomers";
import { setCustomers } from "../redux/actions/customersActions";
import CustomerSales from "../containers/AreaContainers/CustomerSales";
import CustomerDetails from "../containers/AreaContainers/CustomerDetails";
import { setAreas } from "../redux/actions/areasActions";

const Areas = (props) => {
  const [newCustomers, setNewCustomers] = useState(false)
  const [buttonName, setButtonName] = useState('Add New Areas')
  ...
  const customers = useSelector((state) => state.areas.areas);
  ...
  const addCustomerHandler = () => {
    setQuery('')
    if (buttonName == "Add New Areas"){
      setNewCustomers(true)
      setButtonName("Go To Areas")
      setShowProfile(false)
      setSale(false)
      return
    } else if (buttonName == "Go To Areas") {
      setShowProfile(false)
      setSale(false)
      setNewCustomers(false)
      setButtonName("Add New Areas") 
      setUpdate(false)
    }
    ...
    if (data.length > 0) {
      if (query == ""){
        ...
        return data.filter(
          (std) =>
          (std.status == status || status == "All") && (
        std.areaName.toLowerCase().includes(query) ))
      }
    } else {
      return
    }
  ...
  const fetchAreas = async (status) => {

    const response = await axios
    .get("http://127.0.0.1:80/api/v1/areas")
    .catch((err) => {
      alert(err.response.data.message);
    });

  dispatch(setAreas(response.data.data.areas)); 
  if (response.data.data.departments?.length < 1)
  setState("No areas to display!")
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2> {newCustomers ? "Create New Areaa" : 
        showProfile ? "Customer Transactions" : "Areas"}</h2>
        <Button
          variant="contained"
          style={{
          ...
          onClick = {() => {
            if (activeUser.privillages.includes("Add New Areas")){
              addCustomerHandler()
            } else {
